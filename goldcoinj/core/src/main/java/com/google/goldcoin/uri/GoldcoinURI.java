...
/**
 * <p>Provides a standard implementation of a goldcoin URI with support for the
 * following:</p>
 * <p/>
 * <ul>
 ...
 * <p/>
 * <ul>
 * <li>{@code gldcoin:<address>}</li>
 * <li>{@code gldcoin:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 */
public class GoldcoinURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(GoldcoinURI.class);
    ...
    public static final String LITECOIN_SCHEME = "goldcoin";
    ...
    /**
     * Constructs a new GoldcoinURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws GoldcoinURIParseException if the URI is not syntactically or semantically valid.
     */
    public GoldcoinURI(String uri) {
        this(null, uri);
    }
     ...
     * @throws GoldcoinURIParseException If the input fails Litecoin URI syntax and semantic checks.
     */
    public GoldcoinURI(NetworkParameters params, String input) {
        // Basic validation
        ...
        try {
            ...
            throw new GoldcoinURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  gldcoin:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form gldcoin://address?....
        // We support both until Ben fixes his code.

        // Remove the goldcoin scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : gldcoin:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        // the & (%26) in Tom and Jerry gets interpreted as a separator and the label then gets parsed
        // as 'Tom ' instead of 'Tom & Jerry')
        String schemeSpecificPart;
        if (input.startsWith("gldcoin://")) {
            schemeSpecificPart = input.substring("gldcoin://".length());
        }
        else if (input.startsWith("goldcoin://")) {
            schemeSpecificPart = input.substring("goldcoin://".length());
        } else if (input.startsWith("gldcoin:")) {
            schemeSpecificPart = input.substring("gldcoin:".length());
        } else if (input.startsWith("goldcoin:")) {
            schemeSpecificPart = input.substring("goldcoin:".length());
        } else {
            throw new GoldcoinURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?");
        if (addressSplitTokens.length == 0 || "".equals(addressSplitTokens[0])) {
            throw new GoldcoinURIParseException("Missing address");
        }
        ...
        if (addressSplitTokens.length == 1) {
            ...
            if (addressSplitTokens.length == 2) {
                ...
                throw new GoldcoinURIParseException("Too many question marks in URI '" + uri + "'");
            }
        }
    ...
    private void parseParameters(NetworkParameters params, String addressToken, String[] nameValuePairTokens) {
        // Attempt to parse the addressToken as a goldcoin address for this network
        try {
            Address address = new Address(params, addressToken);
            putWithValidation(FIELD_ADDRESS, address);
        } catch (final AddressFormatException e) {
            throw new GoldcoinURIParseException("Bad address", e);
        }
        ...
        for (String nameValuePairToken : nameValuePairTokens) {
            ...
            if (tokens.length != 2 || "".equals(tokens[0])) {
                throw new GoldcoinURIParseException("Malformed gldcoin URI - cannot parse name value pair '" +
                        nameValuePairToken + "'");
            }
    ...
    private void putWithValidation(String key, Object value) {
        if (parameterMap.containsKey(key)) {
            throw new GoldcoinURIParseException("'" + key + "' is duplicated, URI is invalid");
        } else {
    ...
    /**
     * @return The goldcoin Address from the URI
     */
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("GoldcoinURI[");
        boolean first = true;
    ...
    public static String convertToGoldcoinURI(Address address, BigInteger amount, String label, String message) {
        return convertToGoldcoinURI(address.toString(), amount, label, message);
    }

    /**
     * Simple goldcoin URI builder using known good fields.
     *
     * @param address The goldcoin address
     * @param amount  The amount in nanocoins (decimal)
     * @param label   A label
     * @param message A message
     * @return A String containing the goldcoin URI
     */
    public static String convertToGoldcoinURI(String address, BigInteger amount, String label, String message) {
