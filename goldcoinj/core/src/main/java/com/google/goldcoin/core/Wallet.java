...
import com.google.goldcoin.crypto.KeyCrypterScrypt;
import org.goldcoinj.wallet.Protos.Wallet.EncryptionType;
import org.spongycastle.crypto.params.KeyParameter;

import com.google.goldcoin.core.TransactionConfidence.ConfidenceType;
import com.google.goldcoin.core.WalletTransaction.Pool;
import com.google.goldcoin.crypto.KeyCrypter;
import com.google.goldcoin.crypto.KeyCrypterException;
import com.google.goldcoin.store.WalletProtobufSerializer;
...
import static com.google.goldcoin.core.Utils.litecoinValueToFriendlyString;
 ...
 * <p/>
 * <p>Wallets can be serialized using either Java serialization - this is not compatible across versions of goldcoinj,
 ...
 * {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.google.goldcoin.core.Wallet.AutosaveEventListener)}
...
public class Wallet implements Serializable, BlockChainListener {
     ...
     * that handles this in goldcoin C++ is complicated. Satoshis code will not allow you to spend unconfirmed coins,
    ...
    public static class SendResult {
        /**
         * The goldcoin transaction message that moves the money.
         */
    ...
    public static class SendRequest {
         ...
         * is how mining is incentivized in later years of the goldcoin system when inflation drops. It also provides
         * a way for people to prioritize their transactions over others and is used as a way to make denial of service
         * attacks expensive. Some transactions require a fee due to their structure - currently goldcoinj does not
     ...
     * followed by {@link Wallet#completeTx(com.google.goldcoin.core.Wallet.SendRequest)} and returning the requests
     ...
     *
     * @param address   The goldcoin address to send the money to.
     ...
     * {@link Wallet#completeTx(com.google.goldcoin.core.Wallet.SendRequest)} to make it valid.
     *
     ...
     */
    public boolean _completeTx(SendRequest req) {
        lock.lock();
        try {
    ...
    public boolean completeTx(SendRequest req) {
        ...
        try {
            ...
            if (bestChangeOutput != null) {
                ...
                log.info("  with {} coins change", litecoinValueToFriendlyString(bestChangeOutput.getValue()));
            }
            final BigInteger calculatedFee = totalInput.subtract(totalOutput);
            if (calculatedFee.compareTo(BigInteger.ZERO) > 0) {
                log.info("  with a fee of {}", litecoinValueToFriendlyString(calculatedFee));
            }
     ...
     * {@link com.google.goldcoin.core.ECKey#getCreationTimeSeconds()}. This can return zero if at least one key does
    ...
    /**
     * Convenience wrapper around {@link Wallet#encrypt(com.google.goldcoin.crypto.KeyCrypter,
     ...
     * {@link com.google.goldcoin.crypto.KeyCrypterScrypt}.
     *
        ...
        try {
            ...
            for (ECKey key : keychain) {
                if (key.isEncrypted()) {
                    ...
                    // (all goldcoin controlled by that private key is lost forever).
    ...
    /**
     * <p>Convenience wrapper around {@link Wallet#addNewEncryptedKey(com.google.goldcoin.crypto.KeyCrypter,
     ...
     *
     * @param minTransactionsToKeep minumum number of transactions to keep in wallet
     */
    ...
    private class FeeCalculation {
        ...
        public FeeCalculation(SendRequest req, BigInteger value, List<TransactionInput> originalInputs,
           ...
           // double priority = 0.0;
            ...
            BigInteger feesToAdd = BigInteger.ZERO;
            ...
            while (true) {
                ...
                valueNeeded = value.add(fees.compareTo(req.fee) < 0 ? req.fee : fees);
                ...
                /* This will not work because the Wallet does not have enough information to determine the block size

                int count = selection.gathered.size();

                for(int i = 0; i < count; ++i)
                {
                   TransactionOutput txOut = selection.gathered.get(i);
                   BigInteger credit = txOut.getValue();
                   int coinAge = txOut.parentTransaction.getConfidence().getDepthInBlocks();
                   priority = credit.doubleValue() * coinAge;

                }
                boolean free = allowFree(priority);
                if(free)
                {
                    if (lastCalculatedSize < 27000)
                        feesToAdd = BigInteger.ZERO;
                }
                */
                //fees.add(feesToAdd);
                ...
                if (change.compareTo(BigInteger.ZERO) > 0) {
                    ...
                    if (req.ensureMinRequiredFee && Transaction.MIN_NONDUST_OUTPUT.compareTo(change) >= 0) {
                        ...
                        size += changeOutput.litecoinSerialize().length + VarInt.sizeOf(req.tx.getOutputs().size()) - VarInt.sizeOf(req.tx.getOutputs().size() - 1);
                ...
                size += req.tx.litecoinSerialize().length;
            ...
            if (selection3 == null && selection2 == null && selection1 == null) {
                log.warn("Insufficient value in wallet for send: needed {}", litecoinValueToFriendlyString(valueNeeded));
        ...
        private boolean allowFree(double priority)
        {
            // Large (in bytes) low-priority (new, small-coin) transactions
            // need a fee.
            return priority > 100000000 * 576 / 250; // GLDcoin: 576 blocks found a day. Priority cutoff is 1 GLDcoin day / 250 bytes.
